...
if (!exists("projscenario")) projscenario <- "CPDOnly-95-8-150-500"
...
cons_final <- cons_final %>% 
  select(Year, Board.Caliper, Grade, Board.Width, Dia, Wind, Plant, 
         `Plant Name` = PlantName, MRPC2.Text, Die,
         Die.Description, 
         Material, Material.Description, Tons, 
         Die.Tons, Die.Pct, Nbr.Boards, Nbr.Subs, Apparent.Sub,
         Parent.Width = Parent.Roll.Width, Parent.Material, Parent.Material.Descr)

cons_final <- cons_final %>%
  arrange(Year, Board.Caliper, Grade, Board.Width, Dia, Wind, Plant,
          Die, Tons)
...
cons_plt <- cons %>%
  group_by(Year, Caliper = `Board Caliper`, Grade, Dia, Wind,
           `Board Width`, `Parent Roll Width`, Plant, `Plant Name`) %>%
  summarize(Tons = sum(Tons)) %>%
  ungroup()

cons_plt <- cons_plt %>%
  mutate(Plant = paste0(`Plant Name`, " (", comma(round(`Tons`, 0)), ")")) %>%
  group_by(Year, Caliper, Grade, `Board Width`, Dia, Wind) %>%
  mutate(Plants = paste0(Plant, collapse = ", ")) %>%
  select(-Plant, -`Plant Name`) %>%
  ungroup() %>%
  group_by(Year, Caliper, Grade, `Board Width`, Dia, Wind, Plants,
           `Parent Roll Width`) %>%
  summarize(`Roll Tons` = sum(`Tons`)) %>%
  ungroup()

cons_plt <- cons_plt %>%
  mutate(`Trim Width` = `Parent Roll Width` - `Board Width`,	
           `Trim Tons` = (`Roll Tons` * `Parent Roll Width` / `Board Width`) - 
           `Roll Tons` ) %>%
             arrange(Year, Caliper, Grade, desc(`Board Width`))
...
cons_bom_sum <- cons_bom %>%
  group_by(Year, Plant, `Board Caliper`, Grade, Wind, Dia, 
