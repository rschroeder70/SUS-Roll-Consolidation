...
if (!exists("projscenario")) projscenario <- "CPDOnly95"
mYear <- "2017.5"
...
cpd_skus <- opt_detail %>%
  select(Mill = Fac, CalDW, Width = Prod.Width, Grade, Prod.CPD.Web, 
         Prod.CPD.Sheet, Prod.Bev.VMI, Prod.CPD.VMI,
         Prod.Eur, Prod.Common.SKU, Tons = Prod.DMD.Tons) %>%
  mutate(Prod.Type = case_when(.$Prod.CPD.Web ~ "CPD.Web",
                               .$Prod.CPD.Sheet ~ "CPD.Sheet",
                               .$Prod.Bev.VMI ~ "Bev.VMI",
...
sku_summary <- cpd_skus %>%
  ...
  summarize(`2016 Tons` = round(sum(Tons), 0),
            ...
            `Repeat Tons from 2015` = 
              round(sum(ifelse(Prod.Common.SKU, Tons, 0), na.rm = TRUE), 0),
            `Repeat SKUs from 2015` = 
              n_distinct(ifelse(Prod.Common.SKU, Tons, 0)))
...
my_f_table <- setFlexTableWidths(my_f_table, 
                                 widths = c(2, rep(0.8, 2), rep(1, 2)))
print(my_f_table)

# Put the widths into categories
#cpd_skus$cWidth <- cut(cpd_skus$Width, seq(0, max(cpd_skus$Width), .125))
...
wb <- loadWorkbook(file.path(proj_root, paste0(projscenario, mYear, 
                               "ParentSummary.xlsx")))
if (!("Pilot Products 2016" %in% names(wb))) {
  addWorksheet(wb, "Pilot Products 2016")
} else {
  removeWorksheet(wb, "Pilot Products 2016")
  addWorksheet(wb, "Pilot Products 2016")
}

writeData(wb, "Pilot Products 2016", opt_new_pre,
...
# formula (2 == 3).
conditionalFormatting(wb, "Pilot Products 2016", cols = 9, 
                      rows = 3:(nrow(opt_new_pre) + 2),
                      rule = "I2 == I3", style = wh_text_style)
# Same for total parent tolc
conditionalFormatting(wb, "Pilot Products 2016", cols = 12:14, 
                      rows = 3:(nrow(opt_new_pre) + 2),
                      rule = "L2 == L3", style = wh_text_style)
conditionalFormatting(wb, "Pilot Products 2016", cols = 9:14, 
                      rows = 3:(nrow(opt_new_pre) + 2),
                      rule = "$I2 != $I3", style = top_line_style)


addStyle(wb, "Pilot Products 2016", nbrStyle,
        rows = 1:nbr_rows + 2, cols = c(7, 11, 12),
        gridExpand = TRUE, stack = TRUE)

setColWidths(wb, "Pilot Products 2016", cols = 8, widths = "auto")

addStyle(wb, "Pilot Products 2016", rows = 2, cols = c(6, 9:14),
         createStyle(wrapText = TRUE), gridExpand = TRUE,
         stack = TRUE)
addStyle(wb, "Pilot Products 2016", rows = 3:(nrow(opt_new_post) + 2),
         cols = 14, createStyle(halign = "right"), stack = TRUE)

freezePane(wb, "Pilot Products 2016", firstActiveRow = 3)
